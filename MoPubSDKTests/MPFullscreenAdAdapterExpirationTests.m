//
//  MPFullscreenAdAdapterExpirationTests.m
//
...
@interface MPFullscreenAdAdapterExpirationTests : XCTestCase
...
@implementation MPFullscreenAdAdapterExpirationTests
...
- (void)setUp {
    ...
    self.adapter = [MPFullscreenAdAdapter new];
...
- (void)testAdNeverExpiresIfNotMoPubCustomEvent {
    self.adapter = [MPThirdPartyFullscreenAdAdapterMock new]; // overwrite the adapter created in `setUp`
    self.adapter.adapterDelegate = self.adapterDelegateMock;
    // no need to mock `fullscreenAdDelegate` since `MPFullscreenAdAdapter` is a self delegate
