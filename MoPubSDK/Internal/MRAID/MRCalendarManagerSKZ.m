...
@interface MRCalendarManagerSKZ ()
...
@implementation MRCalendarManagerSKZ
...
- (id)initWithDelegate:(id<MRCalendarManagerDelegateSKZ>)delegate
{
    if (self = [super init]) {
        _delegate = delegate;
...
{
    // XXX:
    [_eventEditViewController setEditViewDelegate:[MPLastResortDelegateSKZ sharedDelegate]];
}
...
- (void)createCalendarEventWithParameters:(NSDictionary *)parameters
{
    if (!self.eventEditViewController) {
        self.eventEditViewController = [[MPInstanceProviderSKZ sharedProvider] buildEKEventEditViewControllerWithEditViewDelegate:self];
    }
...
- (void)presentCalendarEditor:(EKEventEditViewController *)editor
{
    [[self.delegate viewControllerForPresentingCalendarEditor] mp_presentModalViewControllerSKZ:editor
