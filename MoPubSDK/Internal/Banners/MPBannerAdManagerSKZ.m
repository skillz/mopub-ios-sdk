...
@implementation MPBannerAdManagerSKZ
...
- (void)loadAdWithURL:(NSURL *)URL
{
    ...
    URL = (URL) ? URL : [MPAdServerURLBuilderSKZ URLWithAdUnitID:[self.delegate adUnitId]
...
- (void)scheduleRefreshTimer
{
    ...
    if (timeInterval > 0) {
        self.refreshTimer = [[MPInstanceProviderSKZ sharedProvider] buildMPTimerWithTimeInterval:timeInterval
                                                                                       target:self
...
- (void)communicatorDidReceiveAdConfiguration:(MPAdConfigurationSKZ *)configuration
{
    ...
    if (configuration.adType == MPAdTypeUnknown) {
        [self didFailToLoadAdapterWithError:[MPErrorSKZ errorWithCode:MPErrorServerError]];
        return;
    }
    ...
    if (configuration.adType == MPAdTypeInterstitial) {
        ...
        [self didFailToLoadAdapterWithError:[MPErrorSKZ errorWithCode:MPErrorAdapterInvalid]];
        return;
    }
    ...
    if ([configuration.networkType isEqualToString:kAdTypeClear]) {
        ...
        [self didFailToLoadAdapterWithError:[MPErrorSKZ errorWithCode:MPErrorNoInventory]];
        return;
    }

    self.requestingAdapter = [[MPInstanceProviderSKZ sharedProvider] buildBannerAdapterForConfiguration:configuration
                                                                                            delegate:self];
...
{
    if (![self.requestingAdapter isKindOfClass:[MPLegacyBannerCustomEventAdapterSKZ class]]) {
...
{
    if (![self.onscreenAdapter isKindOfClass:[MPLegacyBannerCustomEventAdapterSKZ class]]) {
